...
pa <- list()
for (m in 1:length(val_cell_types)) {

  cell_type <- val_cell_types[m]
  print(cell_type)
  ...
  layout <- rbind(c(1, 2))
  pa[[m]] <- grid.arrange(p1, p2, layout_matrix = layout, widths = c(1.4, 1))

}
 

all_pa <- grid.arrange(grobs = pa, ncol=3, hline = c(1.5), 
                       padding = unit(2.5, "cm"))

ggsave(plot=all_pa, filename="Graph/All_SVV.svg", 
       width=48, h=12)
